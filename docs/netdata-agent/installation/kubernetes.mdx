...
The [Netdata Helm chart](/docs/netdata-agent/installation/kubernetes-helm-chart-reference) installs one `parent` pod for storing metrics and managing alert notifications, plus an additional
...
2. To install Netdata using the `helm install` command, run:
  ...
  For more installation options, please read our [Netdata Helm chart for Kubernetes](/docs/netdata-agent/installation/kubernetes-helm-chart-reference) reference.
