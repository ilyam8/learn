...
> If you want to turn off [automatic updates](/docs/installing#automatic-updates), use the `--no-updates` option. You can find more installation options below.
...
- `--prepare-offline-install-source`: Instead of insallling the agent, prepare a directory that can be used to install on another system without needing to download anything. See our [offline installation documentation](/docs/installing/offline-systems) for more info.
...
```bash
[ "99a4e0510a0c782447e0c0f522b5cdcf" = "$(curl -Ss https://my-netdata.io/kickstart.sh | md5sum | cut -d ' ' -f 1)" ] && echo "OK, VALID" || echo "FAILED, INVALID"
```
