...
The number of charts displaying system metrics is limited, but you can use any of Netdata's [external plugins](/docs/contributing/external-plugins) to monitor any services you might have installed on your macOS system. 
You could also use a macOS system as the parent node in a [streaming configuration](/docs/streaming/streaming-configuration-reference).
...
> Community-maintained packages _may_ receive support from Netdata, but are only a best-effort affair. Learn more about [Netdata's platform support policy](/docs/installing/platform-support-policy).

## Install Netdata with our automatic one-line installation script

**Local Netdata Agent installation**
To install Netdata using our automatic [kickstart](/docs/installing#automatic-one-line-installation-script) open a new terminal and run:
...
The `kickstart.sh` script accepts additional parameters to automatically [connect](/docs/configuring/connect-agent-to-cloud) your node to Netdata
...
- `--claim-proxy`: Specify a proxy to use when connecting to the cloud in the form of `http://[user:pass@]host:ip` for an HTTP(S) proxy.
  See [connecting through a proxy](/docs/configuring/connect-agent-to-cloud#connect-through-a-proxy) for details.
...
If you experience issues while claiming your node, follow the steps in our [Troubleshooting](/docs/configuring/connect-agent-to-cloud#troubleshooting) documentation.
...
3. Use the same terminal session to install some of Netdata's prerequisites using Homebrew. If you don't want to use [Netdata Cloud](/docs/architecture/the-role-of-netdata-cloud), you can omit `cmake`.
