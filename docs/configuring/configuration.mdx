...
learn_link: "https://learn.netdata.cloud/docs/configuring/configuration"
---
...
installation](/docs/installing#install-on-linux-with-one-line-installer), the **Netdata config
...
- `health_alarm_notify.conf` enables and configures [alert notifications](/docs/alerting/notifications).
...
- `.environment` is a hidden file that describes the environment in which the Netdata Agent is installed, including the
  `PATH` and any installation options. Useful for [reinstalling](/docs/maintaining/reinstall-netdata-agents) or
  [uninstalling](/docs/maintaining/uninstall-netdata-agents) the Agent.
...
See [configure agent containers](/docs/installing/docker#configure-agent-containers).
...
After you make your changes, you need to [restart the Agent](/docs/maintaining/starting-and-stopping-netdata-agents) with `sudo systemctl
