...
[role based access control](/docs/configuring/securing-netdata-agents/role-based-access-model). 
...
to violate any of the [host label naming rules](/docs/configuring/common-configuration-changes#organize-nodes-with-host-labels).
...
> documentation](/docs/streaming/streaming-configuration-reference#securing-streaming-communications) for details. You may also want to use
> [access lists](/docs/configuring/securing-netdata-agents/web-server#access-lists) or [expose the API only to LAN/localhost
> connections](/docs/architecture/security-and-privacy-design#expose-netdata-only-in-a-private-lan).
...
documentation](/docs/alerting/health-configuration-reference#alert-line-host-labels) for more details, and then get creative!

### Host labels in metrics exporting

If you have enabled any metrics exporting via our experimental [exporters](/docs/exporting/exporting-reference), any new host
...
more about exporting, read the [documentation](/docs/exporting/exporting-reference).
