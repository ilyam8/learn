...
[start, stop, or restart the daemon](/docs/maintaining/starting-and-stopping-netdata-agents).
...
You can send commands during runtime via [netdatacli](/docs/configuring/agent-cli).
...
The `collector.log` is the `stderr` of all [collectors](/docs/data-collection/monitor-anything)
 run by `netdata`.
...
then execute this to [restart Netdata](/docs/maintaining/starting-and-stopping-netdata-agents):
