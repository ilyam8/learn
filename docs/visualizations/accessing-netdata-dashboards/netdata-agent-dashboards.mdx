...
sidebar_label: "Netdata Agent Dashboards"
learn_status: "Published"
learn_rel_path: "Visualizations/Accessing Netdata Dashboards"
...
sidebar_position: "1002040"
---
# Legacy Agent dashboard

> ⚠️ You're checking the documentation for the legacy Agent dashboard. For the current version please check [Accessing Netdata Dashboards](/docs/visualizations/accessing-netdata-dashboards).
...
overview](/docs/contributing/web).
...
Netdata uses an [internal, static-threaded web server](/docs/configuring/securing-netdata-agents/web-server) to host the HTML, CSS, and JavaScript
files that make up the local Agent dashboard. You don't have to configure anything to access it, although you can adjust
[your settings](/docs/configuring/securing-netdata-agents/web-server#other-netdataconf-web-section-options) in the `netdata.conf` file, or run Netdata
behind an [Nginx proxy](/docs/configuring/securing-netdata-agents/reverse-proxies/nginx), and so on.
...
broken down into [contexts](/docs/contributing/web#contexts) and
[families](/docs/contributing/web#families).
...
[Increase the metrics retention policy](/docs/configuring/optimizing-metrics-database/change-how-long-netdata-stores-metrics) for your node to see more historical
timeframes.
...
[families](/docs/contributing/web#families) from that section. Netdata automatically
...
For information on creating custom dashboards from scratch, see the [custom dashboards](/docs/miscellaneous/build-a-custom-dashboard-html-page) or
[Atlassian Confluence dashboards](/docs/exporting/atlassian-confluence-dashboards) guides.
