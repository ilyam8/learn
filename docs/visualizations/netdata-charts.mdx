---
custom_edit_url: "https://github.com/netdata/netdata/edit/master/docs/cloud/visualize/interact-new-charts.md"
...
- On highlight, get easy access to [Metric Correlations](/docs/ml-and-troubleshooting/metric-correlations) to see other metrics with similar patterns
...
[Overview tab](/docs/monitor-your-infrastructure/home-overview-and-single-node-view), Single Node tab and
on your [Custom Dashboards](/docs/visualizations/custom-dashboards).
...
[write new alerts](/docs/alerting/health-configuration-reference), or play around
with the [API](/docs/rest-api/api).
...
Netdata also relies on contexts for [alert configuration](/docs/alerting/health-configuration-reference) (the [`on` line](/docs/alerting/health-configuration-reference#alert-line-on)).
...
- **Netdata icon**: this indicates that data is continuously being updated, this happens if [Time controls](/docs/monitor-your-infrastructure/visualization-date-and-time-controls#time-controls) are in Play or Force Play mode.
...
For more details on each, you can refer to our Agent's HTTP API details on [Data Queries - Data Grouping](/docs/rest-api/queries#data-grouping).
...
Your charts are controlled using the available [Time controls](/docs/monitor-your-infrastructure/visualization-date-and-time-controls#time-controls).
...
- Running [metric correlations](/docs/ml-and-troubleshooting/metric-correlations) to filter metrics that also show something different in the selected period, vs the previous one
