...
learn_link: "https://learn.netdata.cloud/docs/architecture/deployment-strategies"
---
...
The stand-alone setup is configured out of the box with reasonable defaults, but please consult our [configuration documentation](/docs/configuring) for details, including the overview of [common configuration changes](/docs/configuring/common-configuration-changes).
...
For the Parent, besides setting up streaming, the example will also provide an example configuration of multiple [tiers](https://github.com/netdata/netdata/blob/master/src/database/engine/README.md#tiering) of metrics [storage](/docs/configuring/optimizing-metrics-database/change-how-long-netdata-stores-metrics), for 10 children, with about 2k metrics each.
