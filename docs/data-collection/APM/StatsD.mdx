...
Private charts are controlled with `create private charts for metrics matching = *`. This setting accepts a space-separated list of [simple patterns](/docs/contributing/libnetdata/simple-patterns). Netdata will create private charts for all metrics **by default**.
...
> You can read more about how the Netdata Agent organizes information in the relevant [documentation](/docs/contributing/web)
...
-   `metrics` is a Netdata [simple pattern](/docs/contributing/libnetdata/simple-patterns). This pattern should match all the possible StatsD metrics that will be participating in the application `myapp`.
...
`pattern` is a keyword. When set, `METRIC` is expected to be a Netdata [simple pattern](/docs/contributing/libnetdata/simple-patterns) that will be used to match all the StatsD metrics to be added to the chart. So, `pattern` automatically matches any number of StatsD metrics, all of which will be added as separate chart dimensions.
...
Find more details about family and context in our [documentation](/docs/contributing/web#families).
...
- **pattern**: A keyword that tells the StatsD server the `METRIC` string is actually a 
  [simple pattern](/docs/contributing/libnetdata/simple-patterns). 
...
To edit the section, please follow the Netdata [documentation](/docs/visualizations/accessing-netdata-dashboards/netdata-agent-dashboards#customizing-the-local-dashboard).
