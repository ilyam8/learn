...
sidebar_label: "eBPF for Systems Monitoring"
...
sidebar_position: "618080"
slug: "/data-collection/linux-systems/ebpf-for-systems-monitoring"
---
...
For hands-on configuration and troubleshooting tips see our [tutorial on troubleshooting apps with eBPF metrics](/docs/miscellaneous/monitor-troubleshoot-and-debug-applications-with-ebpf-metrics).
...
1.  Navigate to the [Netdata config directory](/docs/configuring/configuration#the-netdata-config-directory).
    ```bash
    cd /etc/netdata
    ```

2.  Use the [`edit-config`](/docs/configuring/configuration#use-edit-config-to-edit-configuration-files) script to edit `netdata.conf`.
...
1.  Navigate to the [Netdata config directory](/docs/configuring/configuration#the-netdata-config-directory).
    ...
    cd /etc/netdata
    ```
2.  Use the [`edit-config`](/docs/configuring/configuration#use-edit-config-to-edit-configuration-files) script to edit [`ebpf.d.conf`](https://github.com/netdata/netdata/blob/master/collectors/ebpf.plugin/ebpf.d.conf).
...
[`apps.plugin`](/docs/data-collection/apm/application-monitoring). This integration helps you understand how specific applications
...
[`cgroups.plugin`](/docs/data-collection/containers-and-vms/cgroups). This integration helps you understand how a specific cgroup
...
-   `cachestat`: Netdata's eBPF data collector creates charts about the memory page cache. When the integration with
    [`apps.plugin`](/docs/data-collection/apm/application-monitoring) is enabled, this collector creates charts for the whole host _and_
...
1.  Navigate to the [Netdata config directory](/docs/configuring/configuration#the-netdata-config-directory).
    ...
    cd /etc/netdata
    ```
2.  Use the [`edit-config`](/docs/configuring/configuration#use-edit-config-to-edit-configuration-files) script to edit a thread configuration file. The following configuration files are available:
...
[simple patterns](/docs/contributing/libnetdata/simple-patterns). The `ports`, and `ips` settings accept negation (`!`) to deny
...
Next, [restart Netdata](/docs/maintaining/starting-and-stopping-netdata-agents) with
...
The eBPF plugin has a [function](/docs/visualizations/functions) named
