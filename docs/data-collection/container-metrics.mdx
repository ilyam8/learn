...
sidebar_position: "606000"
learn_link: "https://learn.netdata.cloud/docs/data-collection/container-metrics"
---
...
[Helmchart](/docs/installing/kubernetes) to automate deployment, collectors for k8s agent services, and
...
[dockerd](/docs/data-collection/containers-and-vms/docker) or [Docker
Engine](/docs/data-collection/containers-and-vms/docker-engine). You can find all
...
collector](/docs/data-collection/databases/mysql) to look at `127.0.0.0:3306` for
MySQL metrics:
...
metrics](/docs/data-collection/databases/mysql#charts) as well.
...
Netdata can even [run in a Docker container](/docs/installing/docker) itself, and then collect metrics about the
...
-   A [service discovery plugin](https://github.com/netdata/agent-service-discovery), which discovers and creates
    ...
    collector](/docs/data-collection/generic-data-collection/prometheus-endpoint). With these
...
-   A [Kubelet collector](/docs/data-collection/kubernetes/kubelet), which runs
    on each node in a k8s cluster to monitor the number of pods/containers, the volume of operations on each container,
    and more.
-   A [kube-proxy collector](/docs/data-collection/kubernetes/kubeproxy), which
